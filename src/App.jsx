...
const BASE = "spring-boot-bankapi-production.up.railway.app";
...
function fmtDate(d) {
  if (!d) return "";
  // Spring LocalDateTime → [year, month, day, hour, minute, second] or "2026-03-10T14:30:00"
  if (Array.isArray(d)) {
    const [y, mo, day, h = 0, m = 0] = d;
    return `${String(day).padStart(2,"0")}.${String(mo).padStart(2,"0")}.${y} ${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}`;
  }
  const dt = new Date(d);
...
const NAV_ADMIN = [
  { id: "adminCustomers", icon: "◎", label: "Wszyscy klienci" },
  { id: "adminAccounts", icon: "▦", label: "Wszystkie konta" },
];
  ...
  return (
    <aside style={{ width: 230, flexShrink: 0, background: C.surface, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", minHeight: "100vh", position: "sticky", top: 0 }}>
      ...
      <nav style={{ flex: 1, padding: "12px 10px", overflowY: "auto" }}>
        ...
        {customer?.role === "ADMIN" && (
          <>
            <div style={{ fontSize: 10, color: C.textDim, letterSpacing: "0.12em", textTransform: "uppercase", padding: "14px 12px 6px", marginTop: 6, borderTop: `1px solid ${C.border}` }}>Panel admina</div>
            {NAV_ADMIN.map(p => {
              const a = page === p.id;
              return (
...
function Accounts({ customer, selAccount, setSelAccount }) {
  ...
  useEffect(() => {
    ...
    api(`/api/account/${selAccount.id}/transactions`)
      .then(d => setTxns((d ?? []).sort((a, b) => new Date(b.date) - new Date(a.date))))
      .catch(() => setTxns([]))
                ...
                return (
                  <div key={tx.id ?? i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: `1px solid ${C.border}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ color: C.textSub, fontSize: 11, marginTop: 2, fontFamily: "monospace" }}>{fmtDate(tx.date)}</div>
                    </div>
                    <div style={{ fontFamily: "monospace", fontWeight: 600, fontSize: 14, color: txColor[tx.type] ?? C.text, whiteSpace: "nowrap" }}>
                      {pos ? "+" : "−"}{money(tx.amount, tx.currency ?? selAccount.currency)}
                    </div>
                  </div>
                );
              })}
            ...
            {shown.map((tx, i) => (
              <div key={tx.id ?? i} style={{ display: "grid", gridTemplateColumns: "80px 1fr 130px 130px 120px", padding: "13px 20px", borderBottom: `1px solid ${C.border}`, fontSize: 13, alignItems: "center", transition: "background .1s" }}
                ...
                <span style={{ textAlign: "right", fontFamily: "monospace", fontWeight: 600, color: tColor[tx.type] ?? C.text }}>
                  {(tx.type === "RECEIVE" || tx.type === "DEPOSIT") ? "+" : "−"}{money(tx.amount, tx.currency)}
                </span>
              </div>
            ))}
          </div>
...
function AdminCustomers() {
  ...
  function startEdit(c) {
    setEditId(c.id);
    setEditForm({ username: c.username, email: c.email });
  }

  async function saveEdit() {
    setSaving(true);
    try {
      await api("/api/customers", { method: "PUT", body: JSON.stringify({ id: editId, ...editForm }) });
      setEditId(null);
      load();
    } catch (e) { alert("Błąd: " + e.message); }
    finally { setSaving(false); }
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
          ...
          {list.map(c => (
            <div key={c.id} style={{ borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr auto", padding: "14px 20px", alignItems: "center", fontSize: 13, transition: "background .1s" }}
                onMouseEnter={e => (e.currentTarget.style.background = C.surfaceHigh)}
                onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <Btn variant="ghost" sm onClick={() => editId === c.id ? setEditId(null) : startEdit(c)}>{editId === c.id ? "Anuluj" : "Edytuj"}</Btn>
                  <Btn variant="red" sm onClick={() => del(c.id)}>Usuń</Btn>
                </div>
              </div>
              {editId === c.id && (
                <div style={{ background: C.bg2, padding: "14px 20px", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end", borderTop: `1px solid ${C.border}` }}>
                  <div style={{ flex: 1, minWidth: 140 }}><Field label="Użytkownik" value={editForm.username} onChange={e => setEditForm(f => ({ ...f, username: e.target.value }))} /></div>
                  <div style={{ flex: 1, minWidth: 140 }}><Field label="Email" value={editForm.email} onChange={e => setEditForm(f => ({ ...f, email: e.target.value }))} /></div>
                  <Btn onClick={saveEdit} disabled={saving}>{saving ? "Zapisywanie…" : "Zapisz zmiany"}</Btn>
                </div>
              )}
            </div>
          ))}
...
function AdminAccounts() {
  ...
  const [err, setErr]           = useState("");
  const [editId, setEditId]     = useState(null);
  const [editForm, setEditForm] = useState({ balance: "", currency: "PLN" });
  const [saving, setSaving]     = useState(false);

  const load = useCallback(async () => {
    setLoading(true); setErr("");
    try { setList(await api("/api/accounts") ?? []); }
    catch (e) { setErr(e.message); }
    finally { setLoading(false); }
  }, []);
  ...
  async function del(id) {
    ...
    try { await api(`/api/accounts/${id}`, { method: "DELETE" }); load(); }
    catch (e) { alert("Błąd: " + e.message); }
  }

  function startEdit(acc) {
    setEditId(acc.id);
    setEditForm({ balance: acc.balance, currency: acc.currency });
  }

  async function saveEdit(acc) {
    setSaving(true);
    try {
      await api("/api/accounts", { method: "PUT", body: JSON.stringify({ ...acc, balance: Number(editForm.balance), currency: editForm.currency }) });
      setEditId(null);
      load();
    } catch (e) { alert("Błąd: " + e.message); }
    finally { setSaving(false); }
  ...
  return (
    <Shell title="Wszystkie konta" subtitle="Podgląd i zarządzanie kontami w systemie">
      {loading && <Spinner />}
      {err && <ErrBox msg={err} retry={load} />}
      {!loading && !err && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 1fr auto", padding: "11px 20px", borderBottom: `1px solid ${C.border}`, fontSize: 10, color: C.textSub, letterSpacing: "0.12em", textTransform: "uppercase" }}>
            <span>ID</span><span>Nr konta</span><span>Typ</span><span>Saldo</span><span>Akcja</span>
          </div>
          {list.length === 0 && <div style={{ textAlign: "center", color: C.textSub, padding: 46, fontSize: 13 }}>Brak kont</div>}
          {list.map(acc => (
            <div key={acc.id} style={{ borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 1fr auto", padding: "14px 20px", alignItems: "center", fontSize: 13, transition: "background .1s" }}
                onMouseEnter={e => (e.currentTarget.style.background = C.surfaceHigh)}
                onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                <span style={{ fontFamily: "monospace", color: C.textSub, fontSize: 12 }}>#{acc.id}</span>
                <span style={{ fontFamily: "monospace", fontSize: 12 }}>···· {acc.accountNumber}</span>
                <AccBadge type={acc.type} />
                <span style={{ fontFamily: "monospace", fontWeight: 600 }}>{money(acc.balance, acc.currency)}</span>
                <div style={{ display: "flex", gap: 6 }}>
                  <Btn variant="ghost" sm onClick={() => editId === acc.id ? setEditId(null) : startEdit(acc)}>{editId === acc.id ? "Anuluj" : "Edytuj"}</Btn>
                  <Btn variant="red" sm onClick={() => del(acc.id)}>Usuń</Btn>
                </div>
              </div>
              {editId === acc.id && (
                <div style={{ background: C.bg2, padding: "14px 20px", display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end", borderTop: `1px solid ${C.border}` }}>
                  <div style={{ flex: 1, minWidth: 140 }}><Field label="Saldo" type="number" value={editForm.balance} onChange={e => setEditForm(f => ({ ...f, balance: e.target.value }))} /></div>
                  <div style={{ flex: 1, minWidth: 140 }}>
                    <Sel label="Waluta" value={editForm.currency} onChange={e => setEditForm(f => ({ ...f, currency: e.target.value }))}>
                      <option value="PLN">PLN</option>
                      <option value="EUR">EUR</option>
                      <option value="USD">USD</option>
                    </Sel>
                  </div>
                  <Btn onClick={() => saveEdit(acc)} disabled={saving}>{saving ? "Zapisywanie…" : "Zapisz zmiany"}</Btn>
                </div>
              )}
            </div>
          ))}
